...
| Release from U-Boot, then boot Linux with the KMS driver blacklisted | **blocked** — the release is clean at the U-Boot prompt, but Linux then locks the SoC in *early kernel boot*, right after `Machine model:` and long before any display code. Three orderings tried, three locks: **a live MIPS window layer and a booting Linux cannot coexist by any ordering we can construct.** |
