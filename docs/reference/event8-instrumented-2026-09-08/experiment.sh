#!/bin/sh
set -eu
OUT=/root/event8-result-20260908
mkdir -p "$OUT"
...
snap() {
  label=$1
  mark "$label"
  for off in 0x80 0x84 0x88 0x8c 0x90 0x94 0x98 0x9c; do printf "%s %s\n" "$off" "$(busybox devmem $((0x4e340000+off)) 32)"; done > "$OUT/$label.trace"
  cat "$OUT/$label.trace"
