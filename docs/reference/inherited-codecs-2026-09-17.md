...
VP8 in hardware. Neither should be removed. The one gap is **field-coded**
MPEG-2, which the driver refuses outright for a reason that is explicit in
upstream cedrus.
...
### FIELD pictures do NOT work, and the reason is exact
...
- Interlaced MPEG-2 works as FRAME pictures. FIELD pictures are **rejected** —
  see above; the driver does not offer the hold-capture-buffer capability for
  MPEG-2, so a field-coded stream cannot be decoded at all.
