...
| Video on the panel | ✅ **decoded H.264 plays on the projector** (2026-08-09..12), correct colour and geometry, operator-confirmed. **Direct YUV scanout works** — NV12 straight to the display with no CPU colour conversion, via the vendor's plane-address path (`0x05600070` Y / `0x05600084` C, format code 3 at `0x05600011`, dirty latch `0x0560006c`); streaming 57.77 fps. Presentation is **vsync-limited at 58.93 fps**; decode alone runs at **268 fps**. Double buffering confirmed working. **DECD (the vendor display driver) is enabled as a module** and submits frames with a real fence — two bugs had to be fixed in it first (`queue_work_on(4,…)` on a 4-core SoC, and an out-of-bounds write in the vsync handler). One question is open: whether the DECD-submitted frame reaches the panel. Full state and next steps at the top of [video-decode.md](video-decode.md) |
